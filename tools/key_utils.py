"""
Утилиты для управления API-ключами (копия billing/utils.py для CLI)
...
KEYS_FILE = Path(__file__).parent.parent / "billing" / "keys.json"
...
def validate_key_format(key: str) -> bool:
    ...
    # Проверяем формат: sk- + символы (поддерживаем тестовые ключи)
    if not key.startswith('sk-'):
        return False
    
    # Для тестовых ключей разрешаем более гибкий формат
    if key.startswith('sk-test-') or key.startswith('sk-prod-') or key.startswith('sk-demo-'):
        return len(key) >= 8  # Минимальная длина
    
    # Для обычных ключей: sk- + 48 символов (буквы и цифры)
